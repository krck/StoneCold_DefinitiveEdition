...
#ifndef STONECOLD_ECS_ENTITYCOMPONENTARRAY_H
#define STONECOLD_ECS_ENTITYCOMPONENTARRAY_H
...
public:
	...
	virtual void DestroyEntity(scEntityId entityId) = 0;
};
...
public:
	EntityComponentArray(size_t ms) noexcept
		: _entityIndexMap(std::unordered_map<scEntityId, size_t>()), _maxSize(ms), _size(0), _data(new T[ms]) {
		// Always initialize the array with empty values
		for (size_t i = 0; i < _maxSize; i++)
			_data[i] = T();
	}

	inline T& operator[](scEntityId entityId) { return _data[_entityIndexMap[entityId]]; }
	inline const T& operator[](scEntityId entityId) const { return _data[_entityIndexMap.at(entityId)]; }

	T& at(scEntityId entityId) {
	...
	const T& at(scEntityId entityId) const {
		if (_entityIndexMap.find(entityId) != _entityIndexMap.end())
			return _data[_entityIndexMap.at(entityId)];
		throw std::out_of_range("Index not available");
	}
	...
	//
	bool insert(scEntityId entityId, T component) {
		if (_size >= _maxSize)
			return false;
	...
	//
	bool erase(scEntityId entityId) {
		if (_entityIndexMap.find(entityId) == _entityIndexMap.end())
			return false;
	...
	inline void DestroyEntity(scEntityId entityId) override { erase(entityId); }

	inline const T* GetRawData() const noexcept { return _data; }

	inline const std::unordered_map<scEntityId, size_t>& GetEntityIndexMap() const noexcept { return _entityIndexMap; }
...
private:
	std::unordered_map<scEntityId, size_t> _entityIndexMap;
	size_t _maxSize;
	size_t _size;
	T* _data;
};
